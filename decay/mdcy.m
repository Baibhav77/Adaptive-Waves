function [ gr, gnr ] = mdcy( rad, dip, ref, mu, lam, l, rin, T0, nrm )
...
% -------------------------------------------------------------------------
[ Ie, Im ] = I_abs( rad, d, ref, lam, l, T0, rin ); % integrals
% -------------------------------------------------------------------------
T.te = T0.tm;    T.tm = T0.te;    T.me = T0.mm;    T.mm = T0.me;
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------   
    if di == numel(ref) % outside the sphere Eq. (72)        
         fel =  Sx + (T.te(:,end,2,1)./T.te(:,end,1,1)).* xix;
        dfel = dSx + (T.te(:,end,2,1)./T.te(:,end,1,1)).*dxix;
         fml =  Sx + (T.tm(:,end,2,1)./T.tm(:,end,1,1)).* xix;        
    elseif di == 1 % inside the core Eq. (71)        
         fel =  Sx./T.me(:,1,2,2);
        dfel = dSx./T.me(:,1,2,2);
         fml =  Sx./T.mm(:,1,2,2);        
    else % within a shell, except core Eqs. (69)-(70)        
